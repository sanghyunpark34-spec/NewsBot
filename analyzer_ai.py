...
def process_ai_score():
    print("🤖 AI 분석(30개 대형 배치 처리)을 시작합니다...")
    ...
    try:
        ...
        ai_weight, base_weight, engine_choice = 0.55, 0.45, "유료 Claude"

    # 시트 기반 페르소나 로드 및 사내 축적 핵심 스크리닝 명세 결합
    base_persona, rubric_prompt = get_persona_and_rubric()
    combined_system_prompt = f"{base_persona}\n\n{EXECUTIVE_SCREENING_MANDATE}"
    
    # 💡 [구조 확장] 상위 20개에서 30개 정예 배치 그룹으로 정밀 확장
    target_rows = rows[1:31]
    remaining_rows = rows[31:]
    
    print(f"⚖️ 가중치 설정 반영 (AI 비중: {ai_weight_pct}%, 기초점수 비중: {round(base_weight*100, 1)}%)")
    print(f"📊 총 {len(rows)-1}개의 기사 중 상위 {len(target_rows)}개 그룹을 최고 보안 마스터 프롬프트로 일괄 채점합니다. (엔진: {engine_choice})")

    batch_prompt = f"{rubric_prompt}\n\n다음 {len(target_rows)}개 기사 제목을 분석하여, 한화 금융 투자 스크리닝 명세에 근거해 각각 0~100점 사이로 채점해줘.\n형식은 반드시 JSON 정형 데이터로만 응답해줘: {{'1': 점수, '2': 점수, ...}}\n\n기사 리스트:\n"
