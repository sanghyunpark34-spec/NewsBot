...
if not is_manual:
    if now_kst.hour != 16:
        print(f"자동 실행: {now_kst.strftime('%H:%M')}은 16시가 아닙니다. 종료합니다.")
        sys.exit(0)
else:
...
keywords = ["금융권 M&A", "보험사 자본확충", "대체투자 사모펀드", "금융 지배구조 개편", "한화생명 전략", "저축은행 M&A", "금융권 지분투자", "금융 건전성"]
...
if final_articles:
    ...
    print(f"총 {len(final_articles)}건 발송 완료")
else:
    print("전송할 기사가 없습니다.")
